########################################################################################################################
#!!
#! @description: Get frontend EPG
#!
#! @input rspsubtree: See all child elements
...
#!!#
########################################################################################################################
namespace: cisco_aci.app_profiles_endpoint_groups
flow:
  outputs:
  ...
  inputs:
    - rspsubtree:
        required: false
    - url: "${get_sp('cisco_aci_base_url')+'/api/mo/uni/tn-globoTenant/ap-AP_GLOBO/epg-EPG_FRONTEND.json'}"
    - authentication_auth_type: "${get_sp('cisco_aci_auth_type')}"
  name: get_frontend_epg
  results:
    - SUCCESS
    - FAILURE
