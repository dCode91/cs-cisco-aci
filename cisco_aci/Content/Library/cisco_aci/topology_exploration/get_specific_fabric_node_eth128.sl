########################################################################################################################
#!!
#! @description: Get specific fabric node eth1/28
#!
#! @input rspsubtree: See all child nodes
...
flow:
  ...
  inputs:
    - rspsubtree:
        required: false
    - url: "${get_sp('cisco_aci_base_url')+'/api/mo/topology/pod-1/node-101/sys/phys-[eth1/28].json'}"
    - authentication_auth_type: "${get_sp('cisco_aci_auth_type')}"
  name: get_specific_fabric_node_eth128
  results:
    - SUCCESS
    - FAILURE
