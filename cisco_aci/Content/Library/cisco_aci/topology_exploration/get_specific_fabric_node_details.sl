########################################################################################################################
#!!
#! @description: Get specific fabric node details
#!
...
flow:
  ...
  inputs:
    - rspsubtree:
        required: false
    - url: "${get_sp('cisco_aci_base_url')+'/api/mo/topology/pod-1/node-101/sys.json'}"
    - authentication_auth_type: "${get_sp('cisco_aci_auth_type')}"
  name: get_specific_fabric_node_details
  results:
    - SUCCESS
    - FAILURE
